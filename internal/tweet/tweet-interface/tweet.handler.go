...
func (h *TweetHandler) GetCommunityPostsGallery(c *fiber.Ctx) error {
	var req struct {
		CommunityIDs     primitive.ObjectID   `json:"community_ids"`
		ExcludeFilterIDs []primitive.ObjectID `json:"ExcludeFilterIDs"`
	}
	...
	// Llamar al servicio para obtener los posts de las comunidades
	posts, err := h.TweetServise.GetCommunityPostsGallery(c.Context(), req.CommunityIDs, req.ExcludeFilterIDs, idValueToken)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": "Error fetching community posts",
			"error":   err.Error(),
